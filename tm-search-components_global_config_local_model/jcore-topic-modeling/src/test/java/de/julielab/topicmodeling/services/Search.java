...
public class Search {
	...
	public Search() {
		try {
			tm = new MalletTopicModeling("src/test/resources/config_template.xml");
	...
	public TMSearchResult search(Document query, Model model) {
		double probabilityThreshold = tm.xmlConfig.getDouble("search.parameters.parameter"
		...
		Map<String, List<Topic>> queryInstance = tm.inferLabel(query, model);
        ...
        int displayedHits = tm.xmlConfig.getInt("search.results.displayedHits", list.size());
