...
public class TopicIndexer extends JCasAnnotator_ImplBase {
	...
	public void initialize(final UimaContext aContext) throws ResourceInitializationException {
		try {
			...
			tm = new MalletTopicModeling(model_config);
...
//			if (!savedModel.pubmedIdModelId.containsKey(docId)) {
			if (!savedModel.ModelIdpubmedId.containsValue(docId)) {
				Map<String, List<Topic>> result = tm.inferLabel(aJCas, savedModel);
...
//			JCoReTools.addToFSArray(Autodescriptor, documentTopics);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void collectionProcessComplete() throws AnalysisEngineProcessException {
		try {
			tm.saveModel(savedModel, model_file 
					+ "-" + InetAddress.getLocalHost().getHostName()
					+ "-" + ManagementFactory.getRuntimeMXBean().getName());
			log.info("Model with index is written in:" + model_file 
					+ "-" + InetAddress.getLocalHost().getHostName()
					+ "-" + ManagementFactory.getRuntimeMXBean().getName());
